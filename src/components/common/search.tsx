...
const SearchBar = ({handleVisibility}: SearchBarProps) => {
  ...
  const [searchBarState, setSearchBarState] = React.useState<SearchBarProps>({
    visible: false,
    expanded: false,
  });
  ...
  if (!!screenSize.width) {
    if (screenSize.width >= screenBreakPoint) {
      ...
      !searchBarState.visible && setSearchBarState((prevState) => ({...prevState, visible: true}));

      // Make sure it is not expanded and click state is reset
      searchBarState.expanded && setSearchBarState((prevState) => ({...prevState, expanded: false, clicked: false}));
