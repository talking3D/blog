...
const Tile = ({
  index, count, title, slug, locale, isDefault, date, hero_color, hero_image, tags, reading_time,
}: TileProps) => {
          ...
          <div className="flex flex-col justify-between h-full rounded-2xl">
            ...
            <div className="flex justify-between mb-2 px-4  w-full">
              <ul className="flex flex-wrap justify-start text-white ">
                { tags.map((tag) => (
                  <li key={tag} className={`px-3 mr-2 mb-1 before:content-["#"] ${getHeroColor(hero_color).solid} font-roboto text-base rounded-xl`}>{tag}</li>
                )) }
              </ul>
